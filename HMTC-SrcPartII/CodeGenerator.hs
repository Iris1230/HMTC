...
-- CmdIf (edited for CW2)
execute majl env n (CmdIf {ciCondThens = ecs, ciMbElse = mbce}) = do
    lblOver <- newName
    mapM_ (executeEach majl env n lblOver) ecs
    case mbce of
        Nothing -> return ()
        Just ce -> execute majl env n ce
    emit (Label lblOver)
    where
        executeEach :: Int -> CGEnv -> MTInt -> Name
                           -> (Expression, Command)
                           -> TAMCG ()
        executeEach majl env n lblOver (e, c) = do
            lblElse <- newName
            evaluate majl env e
...
evaluate majl env (ExpPrj {epRcd = r, epFld = f, expType = t}) = do
    ...
    emit ADD
-- Conditional expression (added for CW2)
