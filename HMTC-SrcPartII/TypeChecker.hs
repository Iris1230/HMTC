...
chkCmd env (A.CmdIf {A.ciCondThens = ecs, A.ciMbElse = mbce,
                     A.cmdSrcPos=sp}) = do
    ecs'  <- mapM (tcCondThen env) ecs  -- for conditional then part
                                                        -- env |- es : Boolean
                                                        -- env |- cs
    -- check whether there is an else part  
    mbce' <- case mbce of
...
infTpExp env (A.ExpCond {A.ecCond = e1, A.ecTrue = e2,
    ...
    (t1, e2')   <- infNonRefTpExp env e2                    -- Env |- e2 : t1
    (t2, e3')   <- infNonRefTpExp env e3                    -- Env |- e3 : t2
    require (t1 == t2) sp ("Error. They have to have the same \
                          \types; "
